...
const BanUserModal = ({ isOpen, onClose, onConfirm, userName, isBanned }: BanUserModalProps) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-md rounded-2xl border border-zinc-100/80 shadow-2xl p-6 relative z-10 animate-in fade-in zoom-in-95 duration-200 select-none">
        ...
        <div className="flex gap-4 items-start">
          ...
          <div>
            <h3 className="text-lg font-bold text-title">{isBanned ? 'Unban User' : 'Ban User'}</h3>
            <p className="text-sm text-subtitle mt-2 leading-relaxed">
              Are you sure you want to {isBanned ? 'unban' : 'ban'} <span className="font-semibold text-title">{userName}</span>? 
              {isBanned 
                ? ' This user will regain full access to the administration console and services.' 
                : ' Banned users will be blocked from logging into the console or accessing resources.'
              }
            </p>
          </div>
        </div>
